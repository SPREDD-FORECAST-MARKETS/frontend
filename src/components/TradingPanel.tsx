    ...
    <div className="bg-[#0d1117] rounded-lg border border-[#222] overflow-hidden h-full shadow-lg">
      ...
      <div className="p-5 border-b border-[#222]">
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <span className="text-white font-medium">{calculateOdds(marketData)}</span>
        </div>
      </div>
      ...
      <div className="p-5">
        <button
          className={`w-full ${isBuy ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'
            } text-white font-bold py-3 px-4 rounded text-lg transition-all ${quantity <= 0 ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          onClick={onSubmit}
          disabled={quantity <= 0}
        >
